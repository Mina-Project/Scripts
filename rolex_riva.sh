...
cd $pack1
7za a -bso0 -mx=9 -mpass=15 -mmt="-j(nproc)" "$product_name-$codename_device1-$kernel_type-$date1.zip" ./* -x'!'README.md -x'!'LICENCE -xr'!'*.zip
cd ..
...
cd $pack2
7za a -bso0 -mx=9 -mpass=15 -mmt="-j(nproc)" "$product_name-$codename_device2-$kernel_type-$date2.zip" ./* -x'!'README.md -x'!'LICENCE -xr'!'*.zip
cd ..
